...
function reducer(state: PuzzleState, action: PuzzleAction): PuzzleState {
  switch (action.type) {
    case "START": return { currentPos: START_POS, path: [START_POS], hasKey: false, status: "playing" };
...
export default function PuzzleStage({ onComplete, onResult, language }: Props) {
  const [state, dispatch] = useReducer(reducer, { currentPos: null, path: [], hasKey: false, status: "idle" });
  ...
  // Determine animation for last tile
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_auto]">
          ...
          <div className="grid grid-cols-3 grid-rows-3 gap-1 rounded-xl p-4" style={{ width: 450, height: 450, background: "linear-gradient(135deg, hsl(40, 30%, 90%), hsl(40, 20%, 85%))" }}>
            {GRID_MAP.flatMap((row, r) =>
              row.map((tileType, c) => {
                ...
                const isClickable = state.status === "idle" ? r === START_POS[0] && c === START_POS[1] : state.status === "playing" && state.currentPos && isAdjacent(state.currentPos, [r, c]) && !isVisited;
        ...
        <div className="w-72 space-y-4">
          ...
          <div className="space-y-2 rounded-xl border border-border bg-surface p-4 shadow-sm">
            <p className="text-sm text-muted-foreground">{t(language, gameCopy.puzzle.sandTiles)}: <strong className="text-foreground">{visitedSandCount}/{SAFE_SAND_TILES.length}</strong></p>
